...
# API при запуске выполняет backfill поиска и может обратиться к колонкам текущей
# EF-модели. Схема применяется до замены API через Compose, поэтому новый образ
# не сможет стартовать с предыдущей версией базы данных.
