...
Web nginx обновляет адрес `api` через встроенный DNS Docker (`127.0.0.11`),
кэшируя ответ на 5 секунд. Поэтому пересоздание только API со сменой IP не
требует перезапуска Web: маршруты `/api/`, `/health/` и `/media/` используют
общий динамический upstream. Для `server ... resolve` в open-source nginx нужна
версия не ниже 1.27.3; `Dockerfile.web` фиксирует совместимую 1.27.5.
Во время остановки API и обновления DNS возможны кратковременные ошибки 502.
Чтобы применить эту конфигурацию впервые, пересоберите и пересоздайте Web
обычным `deployment/client/deploy.ps1`.
